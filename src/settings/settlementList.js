...
import { Table, Input, Icon, Button, Popconfirm,Row,Col,message,Pagination,Spin,Select,Modal} from 'antd';
...
class settlementList extends React.Component {
       ...
       columns.push(
         {
          ...
          <Button type="primary" onClick={()=>{
             this.addEditParams('/EditSettlement',1,this.stateArray,record)
          }}>编辑</Button>
          <Button onClick={() => {
                    this.ModalShow(record.settlementCode)  
                }   
  ...
  onEidtBotton=()=>{  
      this.addEditParams('/EditSettlement',0,this.stateArray)
  }
render(){
      return (
        <div>
  ...
  resetlModal=()=>{
   ...
   resetMerchantPwd=(nowCode,nowName)=>{
      if(!nowName){
        ...
        changeID(objectJson);
        var obj={
          url:api.resourcePath+api.copySettChan,
          jsonp:objectJson,
          func200:(json)=>{
              global.token = json.token;
              this.BalancePost();
...
columns = [
  ...
  { title: '通道编号', dataIndex: 'settlementCode', key: 'settlementCode'},
